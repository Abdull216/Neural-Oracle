...
app.post('/api/interpret', async (req, res) => {
  ...
  const prompt = `Analyze the spiritual resonance of "${word}" (Abjad: ${abjadValue}). Language: ${lang}. Context: Neural Spiritual Engine. Structure: 1. Quantum Resonance, 2. Spiritual Insight, 3. Practical Advice.`;
  try {
    const result = await ai.models.generateContent({
      model: "gemini-3-flash-preview",
      contents: [{ parts: [{ text: prompt }] }],
      config: { systemInstruction: "You are a master of ancient secrets." }
    });
    res.json({ interpretation: result.text });
  } catch (e) { res.status(500).json({ error: "Veiled." }); }
});

app.post('/api/generate-code', async (req, res) => {
  const { word, abjadValue, lang } = req.body;
  const prompt = `Generate a unique "Power Word" for "${word}" (Abjad: ${abjadValue}). Language: ${lang}.`;
  try {
  ...
  } catch (e) { res.status(500).json({ error: "Redacted." }); }
});
...
<body>
    ...
    <script type="text/babel">
        ...
        const App = () => {
            ...
            const [newCfg, setNewCfg] = useState({ newUser: '', newPass: '', newKey: '' });
            ...
            const handleLogin = async () => {
                ...
                if (r.ok) {
                    ...
                    fetch('/api/admin/data').then(r => r.json()).then(d => {
                        setAdminData(d);
                        setNewCfg({ newUser: d.adminConfig.user, newPass: d.adminConfig.pass, newKey: d.adminConfig.key });
                    });
                        ...
                        {activeTab === 'calc' && (
                            <div className="space-y-12">
                                ...
                                <div className="max-w-xl mx-auto space-y-4">
                                    <div className="flex gap-2 justify-center">{['en', 'ha', 'ar'].map(l => <button key={l} onClick={() => setLang(l)} className={"px-3 py-1 rounded-lg text-[10px] uppercase " + (lang === l ? "bg-[#F27D26]" : "bg-white/5")}>{l}</button>)}</div>
                                    <div className="flex gap-2"><input type="text" value={input} onChange={e => setInput(e.target.value)} placeholder="Enter name..." className="flex-1 bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-xl outline-none focus:border-[#F27D26]/50" /><button onClick={handleCalculate} className="bg-[#F27D26] px-6 rounded-2xl font-bold">SCAN</button></div>
                                </div>
                                ...
                                {result && (
                                    <div className="grid md:grid-cols-2 gap-6">
                                        ...
                                        <div className="glass p-8 rounded-3xl space-y-4"><p className="text-[10px] uppercase text-[#F27D26] font-bold">Interpretation</p><div className="text-sm italic text-white/80 leading-relaxed">{result.interpretation}</div></div>
                                        <div className="md:col-span-2 glass p-8 rounded-3xl space-y-2"><p className="text-[10px] uppercase text-[#F27D26] font-bold">Secret Code</p><div className="text-xs font-mono bg-black/40 p-4 rounded-xl border border-white/5 whitespace-pre-wrap">{result.secretCode}</div></div>
                                    </div>
                                )}
                            </div>
                        )}
                        ...
                        {activeTab === 'contact' && (
                            <div className="max-w-md mx-auto glass p-10 rounded-3xl space-y-8">
                                ...
                                <div className="space-y-4">
                                    {emails.map(e => <div key={e} className="flex items-center gap-3"><span className="text-sm">{e}</span></div>)}
                                    <div className="flex items-center gap-3 text-green-500"><span className="text-sm">{whatsapp}</span></div>
                                </div>
                            </div>
                        )}
                                    ...
                                    <div className="space-y-6">
                                        <div className="glass p-6 rounded-2xl flex justify-between items-center">
                                            ...
                                            <div className="flex gap-2">
                                                <button onClick={() => setEditing(!editing)} className="p-2 bg-white/5 rounded-lg"><SettingsIcon /></button>
                                        ...
                                        {editing ? (
                                            <div className="glass p-8 rounded-3xl space-y-4">
                                                ...
                                                <input type="text" value={newCfg.newUser} className="w-full bg-black/40 border border-white/10 p-3 rounded-lg text-sm" onChange={e => setNewCfg({...newCfg, newUser: e.target.value})} />
                                                <input type="text" value={newCfg.newPass} className="w-full bg-black/40 border border-white/10 p-3 rounded-lg text-sm" onChange={e => setNewCfg({...newCfg, newPass: e.target.value})} />
                                                <input type="text" value={newCfg.newKey} className="w-full bg-black/40 border border-white/10 p-3 rounded-lg text-sm" onChange={e => setNewCfg({...newCfg, newKey: e.target.value})} />
                                                <button onClick={handleUpdate} className="w-full bg-[#F27D26] p-3 rounded-lg font-bold text-xs">SAVE CHANGES</button>
                                            </div>
                                        ) : (
