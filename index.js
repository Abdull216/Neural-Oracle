...
app.get('/', (req, res) => {
  const clientData = {
    abjadMap: ABJAD_MAP, ramlFigures: RAML_FIGURES, gaId: process.env.GA_ID || "G-NEURAL-ENGINE",
    emails: ["allarbaa.cloud@yahoo.com", "abdullahharuna216@gmail.com"], whatsapp: "+234808033353"
  };
...
<body>
                    ...
                    <main className="flex-1 max-w-4xl mx-auto w-full p-6 py-12">
                        ...
                        {activeTab === 'contact' && (
                            <motion.div key="contact" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="max-w-md mx-auto glass p-10 rounded-3xl space-y-8">
                                ...
                                <div className="space-y-4">{emails.map(e => <div key={e} className="flex items-center gap-3"><span className="text-sm font-mono">{e}</span></div>)}<div className="flex items-center gap-3 text-green-500"><span className="text-sm font-mono">{whatsapp}</span></div></div>
                            </motion.div>
                        )}
                        {activeTab === 'admin' && (
                                ...
                                {!isAdmin ? (
                                    ...
                                    <div className="space-y-6">
                                        <div className="glass p-6 rounded-2xl flex justify-between items-center">
                                            ...
                                            <div className="flex gap-2"><button onClick={() => setEditing(!editing)} className="p-2 bg-white/5 rounded-lg hover:bg-white/10 transition-colors"><SettingsIcon /></button><button onClick={() => setIsAdmin(false)} className="p-2 bg-red-500/10 text-red-500 rounded-lg hover:bg-red-500/20 transition-colors"><LogOutIcon /></button></div>
                                        </div>
                                        {editing ? (
                                            <div className="glass p-8 rounded-3xl space-y-4">
                                                ...
                                                <div className="space-y-4">
                                                    ...
                                                    {adminData?.archives.map((cat, i) => (
                                                        <div key={i} className="glass p-6 rounded-3xl space-y-4">
                                                            ...
                                                            <div className="grid gap-4">{cat.codes.map((c, j) => (
  ...
  const finalHtml = html.replace(/GA_ID_HERE/g, clientData.gaId).replace('DATA_HERE', JSON.stringify(clientData));
  res.send(finalHtml);
});
